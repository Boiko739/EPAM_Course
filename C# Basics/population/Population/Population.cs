namespace PopulationTask
{
    public static class Population
    {
        ...
        public static int GetYears(int initialPopulation, double percent, int visitors, int currentPopulation)
        {
            ...
            {
                throw new ArgumentException("The initial population cannot be less or equals 0.", nameof(initialPopulation));
            }
            else if (visitors < 0)
            {
                throw new ArgumentException("The count of visitors cannot be less than 0.", nameof(initialPopulation));
            }
            else if (currentPopulation <= 0)
            {
                throw new ArgumentException("The current population cannot be less or equals 0.", nameof(initialPopulation));
            }
            else if (currentPopulation < initialPopulation)
            {
                throw new ArgumentException("The current population cannot be less than the initial population.", nameof(initialPopulation));
            }
            else if (percent < 0 || percent > 100)
            {
                throw new ArgumentOutOfRangeException(nameof(percent), "The value of percents cannot be less then 0 or more then 100.");
            }
